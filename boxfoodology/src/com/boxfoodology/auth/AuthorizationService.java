...
public class AuthorizationService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(final String email) throws UsernameNotFoundException, DataAccessException {
		try {
			final com.boxfoodology.db.entity.User user = userRepository.findActiveByEmail(email);
			if (user == null) {
	...
	public static com.boxfoodology.db.entity.User getAuthentificatedUser() {
		try {
			com.boxfoodology.db.entity.User boxfoodologyUser = null;
			Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
			if (principal instanceof UserDetails) {
				UserDetails details = (UserDetails) principal;
				boxfoodologyUser = userRepository.findActiveByEmail(details.getUsername());
			}
			return boxfoodologyUser;
		} catch (Exception e) {
